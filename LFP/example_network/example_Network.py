...
connectionProbability = [[0.0, 0.0]]*1
...
if __name__ == '__main__':
    ...
    for name, size in zip(population_names, population_sizes):
        ...
        for cell in network.populations[name].cells:
            ...
            t2=5.0
            if 20>k>=0:            
                syn = Synapse(cell=cell, idx=0, syntype='Exp2Syn',
                              weight=0.006,
                              **dict(tau1=t1, tau2=t2, e=0.))
                syn.set_spike_times_w_netstim(interval=10.,noise=noise,start=200.,number=35)
            elif 40>k>=20:
                syn = Synapse(cell=cell, idx=0, syntype='Exp2Syn',
                              weight=0.006,
                              **dict(tau1=t1, tau2=t2, e=0.))
                syn.set_spike_times_w_netstim(interval=10.,noise=noise,start=220.,number=30)
            elif 60>k>=40:
                syn = Synapse(cell=cell, idx=0, syntype='Exp2Syn',
                              weight=0.006,
                              **dict(tau1=t1, tau2=t2, e=0.))
                syn.set_spike_times_w_netstim(interval=10.,noise=noise,start=240.,number=25)
            elif 80>k>=60:
                syn = Synapse(cell=cell, idx=0, syntype='Exp2Syn',
                              weight=0.006,
                              **dict(tau1=t1, tau2=t2, e=0.))
                syn.set_spike_times_w_netstim(interval=10.,noise=noise,start=260.,number=20)
            elif 100>k>=80:
                syn = Synapse(cell=cell, idx=0, syntype='Exp2Syn',
                              weight=0.006,
                              **dict(tau1=t1, tau2=t2, e=0.))
                syn.set_spike_times_w_netstim(interval=10.,noise=noise,start=280.,number=15)
            k+=1
    ...
    if RANK == 0:
        ...
        draw_lineplot(ax, decimate(OUTPUT[0]['E'], q=16), dt=network.dt*16,
                        T=(100, 600),
                        scaling_factor=3.,
                        vlimround=None,
                        label="hello",
    ...
    for i, (name, pop) in enumerate(network.populations.items()):
        for cell in pop.cells:
            c = 'C0'
            for j in np.arange(0,cell.xstart.shape[0]):
                #print("idx",idx,"cell.xstart.shape",cell.xstart.shape)
                if j==0:
                    lw = 5
                else:
                    lw = 0.5
                ax.plot(np.r_[cell.xstart[j], cell.xend[j]],
                        np.r_[cell.ystart[j], cell.yend[j]],
                        np.r_[cell.zstart[j], cell.zend[j]],c,
                        lw=lw, zorder=-cell.xmid[0]-cell.ymid[0])

        for k in np.arange(0,OUTPUT[0]['E'].shape[0]):
            LFP = decimate(OUTPUT[0]['E'][k,:])
            scale_fac = 30
            zLFP = scale_fac*(LFP-np.mean(LFP))/np.std(LFP)
            tvec = np.arange(0,LFP.shape[0])*0.0625
            if electrode.z[k]<0:
                ax.plot(tvec*8,np.zeros((tvec.shape[0],)),(zLFP-np.mean(zLFP))+electrode.z[k]-scale_fac,'k',lw=0.5)
            else:
                ax.plot(tvec*8,np.zeros((tvec.shape[0],)),(zLFP-np.mean(zLFP))+electrode.z[k]-scale_fac,'k',lw=0.5)
            ...
            #ax.plot([cell.synapses[0].x],[cell.synapses[0].y], [cell.synapses[0].z], \
    ...
    ax.set_xlim(-600,600)
    ax.set_ylim(-600,600)
    fig.savefig(os.path.join(OUTPUTPATH, 'population_3d_aligned.tiff'),
